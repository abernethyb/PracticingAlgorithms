...
namespace validPasswords
{
    class Program
    {
        static void Main(string[] args)
        {
            string input = @"8-11 l: qllllqllklhlvtl
1-3 m: wmmmmmttm
2-4 p: pgppp
...
";
            ...
            string commaInput = input.Replace("\r\n", ",");
            ...
            {
                List<string> halves = inputList[i].Split(": ").ToList();
                string rule = halves[0];
                string password = halves[1];
                string numbers = rule.Split(" ") [0];
                string character = rule.Split(" ") [1];
                List<string> stringNumbersList = numbers.Split("-").ToList();
                List<int> intNumbersList = new List<int> { };

                // Console.WriteLine(numbers);
                // foreach (string half in halves)
                // {
                //List<string> numbersVletter = half.Split(" ").ToList();
                //string numbers = numbersVletter[0];

                // List<string> stringNumbersList = numbers.Split("-").ToList();
                // List<int> intNumbersList = new List<int> { };
                foreach (string number in stringNumbersList)
                {
                    try
                    {
                ...
                // foreach (int number in intNumbersList)
                // {
                //     Console.WriteLine(number);
                // }
                Console.WriteLine($"first number: {intNumbersList[0]}, second number: {intNumbersList[1]} character: {character} paasword: {password}");
                List<char> passwordChar = password.ToList();
                int occurances = 0;
                foreach (char letter in passwordChar)
                // for (int j = 0; j < passwordChar.Count - 1; j++)
                {
                    string stringletter = letter.ToString();
                    Console.WriteLine($"Hello! {letter} hi");
                    if (stringletter == character)
                    {
                        occurances++;
                    }
                }
                if (occurances >= intNumbersList[0] && occurances <= intNumbersList[1])
                {
                    numberOfValidPasswords++;
                }
                Console.WriteLine(numberOfValidPasswords);
                Console.WriteLine("end of itteration");
                //}
